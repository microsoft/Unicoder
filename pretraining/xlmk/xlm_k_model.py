...
@register_model('xlmk')
class XLMKModel(FairseqLanguageModel):
    ...
    def build_model(cls, args, task):
        ...
        encoder = XLMKEncoder(args, task.source_dictionary)
        return cls(args, encoder)

    def forward(self, src_tokens, task=UnicoderTaskSpace.mlm, features_only=False, return_all_hiddens=False, classification_head_name=None, **kwargs):
        if classification_head_name is not None:
            features_only = True
        x, extra = self.decoder(src_tokens, task, features_only, return_all_hiddens, **kwargs)
        if classification_head_name is not None:
...
class XLMKEncoder(FairseqDecoder):
    """RoBERTa encoder.
    ...
    def __init__(self, args, dictionary):
        ...
        if self.args.enable_projection_head:
            ...
            if self.args.enable_word2doc or self.args.enable_doc2word:
                self.projection_head_dic[UnicoderTaskSpace.wiki_definition.name] = nn.ModuleList([
                    ProjectionHead(
    ...
    def forward(self, src_tokens, task=UnicoderTaskSpace.mlm, features_only=False, return_all_hiddens=False,
        ...
        x, extra = self.extract_features(src_tokens, return_all_hiddens=return_all_hiddens, task=task, **unused)
        if not features_only:
            x = self.output_layer(x, masked_tokens=masked_tokens)
        # contrastive loss
        if task.name in self.projection_head_dic:
            layer_idx = self.args.layer_of_cts_loss
            if task == UnicoderTaskSpace.wiki_definition and unused['variable_index'] == 0:
                contrastive_state = index_3d_tensor_with_1d_index(
                    extra["inner_states"][layer_idx].permute(1, 0, 2),
                    unused["start_end_position"][:, 0])
            else:
                contrastive_state = extra["inner_states"][layer_idx].permute(1, 0, 2)[:, 0]
            extra["before_projection_head"] = contrastive_state
            if self.args.enable_projection_head:
                projection_heads = self.projection_head_dic[task.name]
                if isinstance(projection_heads, nn.ModuleList):
                    contrastive_state = projection_heads[unused['variable_index']](contrastive_state)
                else:
                    contrastive_state = projection_heads(contrastive_state, unused['variable_index'] if 'variable_index' in unused else 0)
            extra["contrastive_state"] = contrastive_state
        return x, extra

    def extract_features(self, src_tokens, return_all_hiddens=False, task=UnicoderTaskSpace.mlm, **unused):
...
@register_model_architecture('xlmk', 'xlmk')
...
@register_model_architecture('xlmk', 'xlmk_base')
def roberta_base_architecture(args):
    base_architecture(args)

@register_model_architecture('xlmk', 'xlmk_large')
