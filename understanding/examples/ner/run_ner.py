...
def train(args, train_dataset, model, tokenizer, labels, pad_token_label_id):
    ...
    def logging():
        if args.evaluate_during_training:
            results, preds_results = evaluate(args, model, tokenizer, labels, pad_token_label_id)
    ...
    for _ in train_iterator:
        ...
        if args.logging_each_epoch:
            avg_metric, preds_results = logging()
            logging_loss = tr_loss
            if avg_metric > best_metric:
                best_preds = preds_results
    ...
    for lang in args.language.split(","):
        output_eval_file = os.path.join(args.output_dir, "{}.prediction".format(lang))
        with open(output_eval_file, "w") as writer:
                sentences = best_preds["test-" + lang]
...
def evaluate(args, model, tokenizer, labels, pad_token_label_id, prefix=""):
    ...
    results["dev_avg"] = average_dic([value for key, value in results.items() if key.startswith("dev")])
...
def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode="train", lang="en"):
    ...
    cached_features_file = os.path.join(
        ...
        "cached_{}_{}_{}_{}".format(
            mode, list(filter(None, args.model_name_or_path.split("/"))).pop(), str(args.max_seq_length), lang
        ),
    )
...
def main():
    ...
    if args.do_train:
        ...
        train_dataset = load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode="train", lang=args.train_language)
