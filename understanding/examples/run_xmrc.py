...
def load_and_cache_examples(args, tokenizer, language, split="train", output_examples=False):
    ...
    if os.path.exists(cached_features_file) and not args.overwrite_cache:
        ...
        if not args.data_dir and ((split != "train" and not args.predict_file) or (split == "train" and not args.train_file)):
...
def main():
    ...
    if args.do_train:
        ...
        if not os.path.exists(args.output_dir) and args.local_rank in [-1, 0]:
            os.makedirs(args.output_dir)
        train_dataset = load_and_cache_examples(args, tokenizer, language=args.train_language, split="train", output_examples=False)
